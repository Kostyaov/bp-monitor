...
> **Остання редакція:** 16 квітня 2026 р.  
> **Версія:** `1.3` (Headache Tracking & Chart Correlation)  
...
| 📊 Візуалізація (Графіки) | ✅ OK | Кореляція Тиск + Головний біль |
| 📜 Керування журналом | ✅ OK | Редагування та видалення записів |
| 📥 Експорт даних | ✅ OK | XLSX файли з усіма полями |
| 📱 Мобільна адаптація | ✅ OK | Оптимізовані графіки та модальні вікна |
| 🔍 Розумна аналітика | ⬜ План | Автовизначення аномалій та прогноз |
...
*   **Libraries**:
    *   `Chart.js` — інтерактивні графіки з підтримкою декількох осей (Dual Y-axis).
    *   `SheetJS` — професійний експорт у XLSX.
    *   `Lucide` — набір іконок.

### 📱 Мобільно-орієнтований підхід
Додаток спроектовано за принципом **Mobile-First**:
*   **Адаптивні графіки**: На мобільних автоматично приховуються зайві заголовки осей («mmHg», «Голова %») для максимізації корисної площі.
*   **Керування**: Всі кнопки та елементи введення мають розмір, зручний для керування одним пальцем.
*   **Оптимізація трафіку**: Фото стискаються на стороні клієнта перед відправкою в AI.
...
*Документація оновлена: 16.04.2026 | Версія продукту: `Production 1.3`*
